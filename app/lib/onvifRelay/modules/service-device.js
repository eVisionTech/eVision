...
/* ------------------------------------------------------------------
* Method: setRelayOutputSettings(params[, callback])
* - params:
   RelayOutputToken [ReferenceToken]
   Properties [Properties] 
* ---------------------------------------------------------------- */
OnvifServiceDevice.prototype.setRelayOutputSettings = function(params, callback) {
	let promise = new Promise((resolve, reject) => {
		let soap_body = '<SetRelayOutputSettings xmlns="http://www.onvif.org/ver10/device/wsdl">';

		soap_body += '<RelayOutputToken>' + params['RelayOutputToken'] + '</RelayOutputToken>';
		soap_body += '<Properties>';

		soap_body += '<Mode xmlns="http://www.onvif.org/ver10/schema">' + params['Mode'] + '</Mode>';
		soap_body += '<DelayTime xmlns="http://www.onvif.org/ver10/schema">' + params['DelayTime'] + '</DelayTime>';
		soap_body += '<IdleState xmlns="http://www.onvif.org/ver10/schema">' + params['IdleState'] + '</IdleState>';

		soap_body += '</Properties>';

		soap_body += '</SetRelayOutputSettings>';

		let soap = this._createRequestSoap(soap_body);
		mOnvifSoap.requestCommand(this.oxaddr, 'SetRelayOutputSettings', soap).then((result) => {
			resolve(result);
		}).catch((error) => {
			reject(error);
		});
...
/* ------------------------------------------------------------------
* Method: SetRelayOutputState(params[, callback])
* - params:
   RelayOutputToken [ReferenceToken]
   LogicalState [Properties] 
* ---------------------------------------------------------------- */
OnvifServiceDevice.prototype.SetRelayOutputState = function(params, callback) {
	let promise = new Promise((resolve, reject) => {
		let soap_body = '<SetRelayOutputState xmlns="http://www.onvif.org/ver10/device/wsdl">';

		soap_body += '<RelayOutputToken>' + params['RelayOutputToken'] + '</RelayOutputToken>';
		soap_body += '<LogicalState>';

		soap_body += params['state'] ;

		soap_body += '</LogicalState>';

		soap_body += '</SetRelayOutputState>';

		let soap = this._createRequestSoap(soap_body);
		mOnvifSoap.requestCommand(this.oxaddr, 'SetRelayOutputState', soap).then((result) => {
			resolve(result);
		}).catch((error) => {
			reject(error);
		});
